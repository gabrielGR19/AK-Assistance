...
test("gültiger Termin wird angenommen", () => {
  const p = validiereTermin(gueltig);
  assert.ok(p.ok);
...
test("leerer Titel wird zu 'Ohne Titel' statt abgelehnt", () => {
  const p = validiereTermin({ ...gueltig, titel: "   " });
  assert.ok(p.ok);
  assert.equal(p.wert.titel, "Ohne Titel");
});

test("unbekanntes Label wird abgelehnt", () => {
  const p = validiereTermin({ ...gueltig, label: "urlaub" });
  assert.ok(!p.ok);
  assert.match(p.fehler, /Label/);
});

test("Ende vor Start wird abgelehnt", () => {
  const p = validiereTermin({ ...gueltig, start: "2026-07-27T11:00", ende: "2026-07-27T10:00" });
  assert.ok(!p.ok);
  assert.match(p.fehler, /Ende/);
});

test("Ende gleich Start wird abgelehnt (kein Null-Minuten-Termin)", () => {
  const p = validiereTermin({ ...gueltig, ende: gueltig.start });
  assert.ok(!p.ok);
});

test("Termin über Mitternacht wird abgelehnt", () => {
  const p = validiereTermin({ ...gueltig, start: "2026-07-27T23:00", ende: "2026-07-28T01:00" });
  assert.ok(!p.ok);
...
test("nicht existierendes Datum wird abgelehnt", () => {
  ...
  const p = validiereTermin({ ...gueltig, start: "2026-02-31T09:00", ende: "2026-02-31T10:00" });
  assert.ok(!p.ok);
});
