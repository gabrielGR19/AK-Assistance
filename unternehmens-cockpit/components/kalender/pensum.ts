...
//
// Gezählt werden nur eigene Termine mit einem Arbeitslabel (labels.get(...).arbeit) — Sport,
...
import type { Label, PersonId, Termin } from "../../lib/kalender-typen.ts";
import { ausIso } from "./datum.ts";

export function pensumAm(termine: Termin[], person: PersonId, tagIso: string, labels: Map<string, Label>): number {
  return termine.reduce((summe, t) => {
    if (t.start.slice(0, 10) !== tagIso) return summe;
    if (t.besitzer !== person) return summe;
    if (!labels.get(t.label)?.arbeit) return summe;
